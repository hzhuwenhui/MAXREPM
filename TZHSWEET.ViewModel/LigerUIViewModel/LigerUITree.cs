...
new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore });
        ...
        public static LigerUITree ToEntity(PM_Category category)
        {
            LigerUITree item = new LigerUITree();
            item.id = category.CategoryID;
            item.text = category.CategoryName;
            item.desc = category.Number.ToString();
            return item;
        }
        /// <summary>
        /// 实体转化
        ...
        public static IEnumerable<LigerUITree> ToListViewModel(IEnumerable<PM_Category> listCategorys)
        {
            List<LigerUITree> list = new List<LigerUITree>();
            foreach (PM_Category category in listCategorys)
            {

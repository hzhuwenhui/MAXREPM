    ...
    public class SysAction
    {
        ...
        public static IList<MVCAction> GetAllActionByAssembly(out IList<MVCController> controllers)
        {
            ...
            foreach (var type in types)
            {
                if (type.BaseType.Name == "BaseController")//如果是Controller
                {
                    var AreaName = type.Namespace.ToString();
                    if (AreaName.IndexOf("Areas") != -1)//如果是区域内的Controller
                    {
                        AreaName = AreaName.Substring(AreaName.IndexOf("Areas.") + 6);//把他的区域名称找出来,即命名空间Area关键字后面的名称
                        if (AreaName.IndexOf("Controllers") != -1)//把后缀.controller去掉
                        {
                            AreaName = AreaName.Replace(".Controllers", "");
                        }
                    }
                    var controller = new MVCController();
                   controller.ControllerName = type.Name.Replace("Controller", "");//去除Controller的后缀
                   ...
                   result.AddRange(GetActions(AreaName, type,out  defaultPage));
                   controller.LinkUrl = "/" + AreaName + "/" + controller.ControllerName +"/"+ defaultPage;
        ...
        /// <returns></returns>
        public static IList<MVCAction> GetActions(string AreaName, Type type,out string DefaultPage)
        {
            //默认是Index页面
            DefaultPage = "Index";
            ...
            foreach (var member in members)
            {
                if (member.ReturnType.Name == "ActionResult")//如果是Action
                {
                    ...
                    item.LinkUrl = "/" + AreaName + "/" + item.ControllerName + "/" + item.ActionName;
                    result.Add(item);
